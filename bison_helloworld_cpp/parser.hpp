...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t,  EzAquarii::Command && v)
        : Base (t)
        , value (std::move (v))
      {}
#else
      basic_symbol (typename Base::kind_type t, const  EzAquarii::Command & v)
        : Base (t)
        , value (v)
      {}
#endif
...
switch (yykind)
    ...
    /// Build a parser object.
     Parser  (EzAquarii::Scanner &scanner_yyarg);
    virtual ~ Parser  ();
    ...
    // Implementation of make_symbol for each token kind.
      ...
      make_END ()
      {
        return symbol_type (token::TOKEN_END);
      }
#else
      static
      symbol_type
